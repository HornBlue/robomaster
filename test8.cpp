...
private:
    ...
    Mat preprocess(const Mat& img) {
        ...
        // 扩大红色范围以适应远距离颜色变化
        inRange(hsv, Scalar(0, 60, 30), Scalar(15, 255, 255), red_mask1);
        inRange(hsv, Scalar(155, 60, 30), Scalar(180, 255, 255), red_mask2);
        red_mask = red_mask1 | red_mask2;
        
        // 更强的形态学操作去除红色
        Mat kernel_red = getStructuringElement(MORPH_RECT, Size(5,5));
        ...
        // 4. 自适应形态学操作
        int morph_size = max(1, min(gray.rows, gray.cols) / 30);
        ...
        if (white_pixels < binary.total() * 0.15) {
            binary = 255 - binary;
    ...
    vector<Mat> generateMultiScaleTemplates(const Mat& template_img) {
        ...
        // 生成不同尺度的模板
        vector<double> scales = {0.8, 0.9, 1.1, 1.2};
    ...
    string matchDigit(const Mat& input_img) {
        ...
        if (min_shape_dist < 0.8) { // 放宽阈值适应远距离
            ...
            if (best_match_idx == 0 && target_curves >= 2) {
                ...
                // 数字3需要至少2个弯曲
                if (shape_distances[4] < min_shape_dist + 0.1) {
                    result = "5";
  ...
  for (size_t i = 0; i < light_bars.size(); i++) {
    for (size_t j = i + 1; j < light_bars.size(); j++) {
      ...
      if (distance > 50 && distance < 300 && angle_diff < 20) {
        ...
        // 改进的数字区域提取 - 自适应ROI大小
        float roi_width = max(20.0f, distance * 0.6f);  // 最小宽度20像素
        float roi_height = max(25.0f, avg_height * 1.2f); // 最小高度25像素
        ...
        if (valid_roi.area() > 100) { // 确保ROI足够大
            // 提取数字区域进行识别
