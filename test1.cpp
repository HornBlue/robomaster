...
string TestNode::recognizeNumber(const Mat& number_roi) {
    ...
    // 调整大小以便于模板匹配
    Mat resized_digit;
    resize(digit_roi, resized_digit, Size(30, 50));
    ...
    // 简单的数字识别逻辑
    // 注意：这是一个简化的识别方法，实际应用中可能需要更复杂的算法或机器学习模型
    
    // 1: 通常比较细长
    if (aspect_ratio < 0.5 && area_ratio > 0.1) {
        return "1";
    }
    // 2: 中等宽度，有弯曲
    else if (aspect_ratio > 0.6 && aspect_ratio < 0.9 && area_ratio > 0.15) {
        return "2";
    }
    // 3: 类似2但更圆润
    else if (aspect_ratio > 0.7 && aspect_ratio < 1.0 && area_ratio > 0.15) {
        return "3";
    }
    // 4: 通常有直角
    else if (aspect_ratio > 0.7 && aspect_ratio < 1.1 && area_ratio > 0.2) {
        return "4";
    }
    // 5: 类似2但上部更平
    else if (aspect_ratio > 0.7 && aspect_ratio < 1.0 && area_ratio > 0.18) {
        return "5";
    }
