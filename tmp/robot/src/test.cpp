...
// 替换：峰值特征结构体（砍掉封闭区域，全用投影/峰值）
struct DigitFeatures {
    int row_peak_min, row_peak_max;    // 行峰值数范围（核心区分1/2/3/4/5）
    double aspect_min, aspect_max;     // 宽高比范围（宽/高）
    double col_peak_ratio;             // 列峰值位置阈值（仅4/5用）
    bool is_right_peak;                // 4：右峰，5：左峰
};

// 替换：1-5的峰值特征库（无封闭区域）
DigitFeatures getDigitFeatures(int digit) {
    switch(digit) {
        case 1: return {1,1, 0.1,0.3, 0, false};string TestNode::recognizeNumber(const Mat& number_roi) {
    ...
    // 自适应二值化，提高对不同光照条件的适应性
    Mat binary;
    adaptiveThreshold(blurred, binary, 255, ADAPTIVE_THRESH_GAUSSIAN_C, THRESH_BINARY_INV, 11, 2);
    ...
    // 获取数字的边界框
    cv::Rect num_rect = cv::boundingRect(contours[max_index]);
    
    // 确保边界框在图像范围内
    if (num_rect.x < 0 || num_rect.y < 0 || 
        num_rect.x + num_rect.width > number_roi.cols ||
        num_rect.y + num_rect.height > number_roi.rows) {
        return "unknown";
    }
    
    // 提取数字区域
    cv::Mat digit_roi = binary(num_rect);
    
    // 调整大小以便于分析
    cv::Mat resized_digit;
    cv::resize(digit_roi, resized_digit, cv::Size(40, 60), 0, 0, cv::INTER_NEAREST);
    cv::imshow("bit",resized_digit);
    
    // 确保调整大小后的图像是二值的
    cv::threshold(resized_digit, resized_digit, 127, 255, cv::THRESH_BINARY);

    // 确保图像是8位单通道
    if (resized_digit.type() != CV_8UC1) {
        cv::Mat temp;
        resized_digit.convertTo(temp, CV_8UC1);
        resized_digit = temp;
    }
    ...
    // 手动计算水平和垂直投影
    std::vector<float> horizontal_proj(resized_digit.rows, 0.0f);
    std::vector<float> vertical_proj(resized_digit.cols, 0.0f);
    
    for (int i = 0; i < resized_digit.rows; i++) {
        uchar* row = resized_digit.ptr<uchar>(i);
        for (int j = 0; j < resized_digit.cols; j++) {
            uchar pixel = row[j];
            horizontal_proj[i] += pixel;
            vertical_proj[j] += pixel;
        }
    }
    
    // 归一化
    for (auto& val : horizontal_proj) val /= resized_digit.cols;
    for (auto& val : vertical_proj) val /= resized_digit.rows;
    ...
    // 计算水平投影的平均值
    float h_avg = 0.0f;
    for (float val : horizontal_proj) h_avg += val;
    h_avg /= horizontal_proj.size();
    
    // 计算垂直投影的平均值
    float v_avg = 0.0f;
    for (float val : vertical_proj) v_avg += val;
    v_avg /= vertical_proj.size();
    
    // 计算水平投影的峰值
    for (int i = 1; i < static_cast<int>(horizontal_proj.size()) - 1; i++) {
        if (horizontal_proj[i] > h_avg * 1.5f && 
    ...
    // 计算垂直投影的峰值
    for (int i = 1; i < static_cast<int>(vertical_proj.size()) - 1; i++) {
        if (vertical_proj[i] > v_avg * 1.5f && 
            vertical_proj[i] > vertical_proj[i-1] &&
            vertical_proj[i] > vertical_proj[i+1]) {
            v_peaks++;
        }
    }
    ...
    // 计算数字的质心位置
    cv::Moments m = cv::moments(resized_digit);
    if (m.m00 == 0) {
        return "unknown";
    }
    ...
    // 计算数字的紧密度（面积与边界框面积之比）
    double compactness = max_area / (num_rect.width * num_rect.height);
    ...
    // 基于多种特征的改进数字识别
    // 数字1: 细长，垂直投影峰值少，质心偏上，紧密度较低
    if (aspect_ratio < 0.6 && v_peaks <= 2 && centroid_x_ratio > 0.4 && 
        centroid_x_ratio < 0.6 && compactness < 0.4) {
        return "1";
    }
    // 数字2: 中等宽高比，有弯曲，水平投影有2-3个峰值，质心偏上
    else if (aspect_ratio > 0.6 && aspect_ratio < 0.9 && h_peaks >= 2 && 
             h_peaks <= 3 && centroid_y_ratio < 0.6 && compactness > 0.3) {
        return "2";
    }
    // 数字3: 类似2但更对称，质心居中，紧密度中等
    else if (aspect_ratio > 0.7 && aspect_ratio < 1.0 && 
             centroid_x_ratio > 0.4 && centroid_x_ratio < 0.6 && 
             compactness > 0.35 && h_peaks >= 2) {
        return "3";
    }
    // 数字4: 通常有直角，垂直投影有多个峰值，质心偏左
    else if (aspect_ratio > 0.7 && aspect_ratio < 1.1 && v_peaks >= 3 && 
             centroid_x_ratio < 0.5 && compactness > 0.4) {
        return "4";
    }
    // 数字5: 上部有横线，下部有弯曲，水平投影有多个峰值，质心偏下
    else if (aspect_ratio > 0.7 && aspect_ratio < 1.0 && h_peaks >= 3 && 
             centroid_y_ratio > 0.4 && compactness > 0.35) {
        return "5";
    }
  ...
  // 1：1个行峰+瘦长
        case 2: return {2,2, 0.6,0.8, 0, false};  // 2：2个行峰+中等宽高比
        case 3: return {2,2, 0.6,0.8, 0, false};  // 3：2个行峰+中等宽高比
        case 4: return {1,1, 0.7,1.0, 0.4, true}; // 4：1个行峰+右峰（>0.4）
        case 5: return {1,1, 0.7,1.1, 0.4, false};// 5：1个行峰+左峰（<0.4）
...
// 核心：提取数字自身的投影/峰值特征（抗平移）
void getPeakFeatures(const Mat& binary, const Rect& num_rect, int& row_peak_num, double& col_peak_ratio) {
    // 1. 裁剪出数字自身的区域（只分析数字，不分析背景，抗平移）
    Rect digit_rect = num_rect & Rect(0,0,binary.cols,binary.rows);
    Mat digit_bin = binary(digit_rect);
    if (digit_bin.empty()) {
        row_peak_num = 0;
        col_peak_ratio = 0;
        return;
    ...
    // 2. 行投影：统计每行白色像素数，找行峰值数量
    vector<int> row_pixel(digit_bin.rows, 0);
    for (int r=0; r<digit_bin.rows; r++) {
        row_pixel[r] = countNonZero(digit_bin.row(r));
    }
    // 找行峰值（像素数>行均值的行）
    int row_mean = accumulate(row_pixel.begin(), row_pixel.end(), 0) / digit_bin.rows;
    row_peak_num = 0;
    for (int cnt : row_pixel) {
        if (cnt > row_mean) row_peak_num++;
    }

    // 3. 列投影：统计每列白色像素数，找列峰值位置比例（相对数字自身）
    vector<int> col_pixel(digit_bin.cols, 0);
    for (int c=0; c<digit_bin.cols; c++) {
        col_pixel[c] = countNonZero(digit_bin.col(c));
    }
    // 找列峰值列（像素数最多的列）
    int max_col_pixel = *max_element(col_pixel.begin(), col_pixel.end());
    int peak_col = 0;
    for (int c=0; c<digit_bin.cols; c++) {
        if (col_pixel[c] == max_col_pixel) {
            peak_col = c;
            break;
        }
    }
    // 列峰值位置比例（相对数字自身宽度，0-1，平移不变）
    col_peak_ratio = (double)peak_col / digit_bin.cols;
}

// 替换：峰值特征匹配函数（无封闭区域）
bool matchFeatures(const Mat& binary, int digit, const Rect& num_rect) {
    DigitFeatures f = getDigitFeatures(digit);
    cout << "\n[调试] 匹配数字：" << digit << endl;
    cout << "要求：行峰值数=" << f.row_peak_min << "-" << f.row_peak_max 
         << "，宽高比=" << f.aspect_min << "-" << f.aspect_max << endl;
    if (digit ==4 || digit ==5) {
        cout << "要求：列峰值" << (f.is_right_peak ? "右偏(>0.4)" : "左偏(<0.4)") << endl;
    }
    ...
    // 2. 提取峰值特征（核心）
    int row_peak_num;
    double col_peak_ratio;
    getPeakFeatures(binary, num_rect, row_peak_num, col_peak_ratio);
    cout << "[调试] 实际行峰值数：" << row_peak_num << endl;

    // 3. 行峰值数校验（核心区分1/2/3/4/5）
    if (row_peak_num < f.row_peak_min || row_peak_num > f.row_peak_max) {
        cout << "[失败] 行峰值数不匹配" << endl;
        return false;
    }

    // 4. 4/5的列峰值位置校验（核心区分）
    if (digit == 4 || digit == 5) {
        cout << "[调试] 列峰值位置比例（相对数字自身）：" << col_peak_ratio << endl;
        if ((f.is_right_peak && col_peak_ratio <= f.col_peak_ratio) || 
            (!f.is_right_peak && col_peak_ratio >= f.col_peak_ratio)) {
            cout << "[失败] 列峰值位置不匹配" << endl;
            return false;
        }
    }
...
string TestNode::recognizeNumber(const Mat& number_roi) {
    ...
    // 峰值特征匹配（优先匹配5，避免被3/4误判）
    // 匹配顺序：5→4→2→3→1（先匹配具体的4/5，再匹配泛的2/3/1）
    if (matchFeatures(binary, 5, num_rect)) return "5";
    if (matchFeatures(binary, 4, num_rect)) return "4";
    if (matchFeatures(binary, 2, num_rect)) return "2";
    if (matchFeatures(binary, 3, num_rect)) return "3";
    if (matchFeatures(binary, 1, num_rect)) return "1";
    ...
    double col_peak_ratio;
    getPeakFeatures(binary, num_rect, row_peak_num, col_peak_ratio);
    cout << "行峰值数：" << row_peak_num << endl;
    cout << "宽高比：" << getAspectRatio(num_rect) << endl;
    cout << "列峰值位置比例：" << col_peak_ratio << endl;
    return "unknown";
}
  ...
  for (size_t i = 0; i < light_bars.size(); i++) {
    for (size_t j = i + 1; j < light_bars.size(); j++) {
      ...
      if (distance > 50 && distance < 300 && angle_diff < 20) {
        ...
        // 检测数字区域（装甲板中间区域）
        Rect number_roi(left_center.x + 10, left_center.y - avg_height / 2 - 20, 
                        right_center.x - left_center.x - 20, avg_height+40);
        // 检测数字区域（装甲板中间区域）
